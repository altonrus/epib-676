...
-   **Expert or data-informed estimation**: based on imperfect or semi-applicable data and/or expert opinion, develop reasonable estimates/uncertainty ranges

-   **Calibration:** use data on ***outcomes*** to estimate parameters
...
2.  Structure model to produce measures analagous to targets

3.  Identify model parameters to be targeted
...
-   Asks, how likely is the modeled output given the target data?

-   Higher likelihood $\rightarrow$ modeled output consistent with data
...
## Compute GOF for each parameter set
...
## Plot calibrated model vs. target
...
Can use other likelihood functions

-   Loglikelihood for a different distribution

-   Weighted sum of square error

Can also define \>1 target

-   Judge parameter set based on combined measure, e.g., weighted sum
...
ggplot(t_compare, aes(x=Time, y=value))+
  ...
  geom_point(color="red", aes(y=random_search))+
  ylab("Pr Survive")+
  geom_point(color="green", alpha = 0.25, aes(y=nelder_mead))
```

## Bayesian probabilistic calibration

-   **Deterministic:** "What's the best-fitting parameter set?"

-   **Probabilsitic:** "What joint distribution of parameters is consistent with the uncertainty in my calibration targets"

-   Bayesian methods allow us to also use data on both parameters and outcomes to estimate parameters
...
-   **Transparency:** clearly documenting, justifying, and publicizing model perspective, framing, structure, parameters, assumptions, limitations

-   **Validation**:judging how accurately a model captures reality

## Model repurposing

-   Some models developed as multipurpose

    -   Oncosim (Canada) or MISCAN (USA)

-   In other cases, we can get more 'mileage' out of a model by applying to new decision problem

-   Must consider how decision-relevant trade-offs differ from original intent

-   A model can have different level of validity for different application
...
-   Model and assumptions reflect current evidence as judged by domain experts (e.g. clinicians) and policy makers
...
-   Internal or technical validity
...
-   How consistent is your model outputs with other models of same decision problem or biological/social phenomena?
...
-   Comparison to real-world data (e.g., trial or epidemiological data)
...
-   Steps
    ...
    -   Identify data sources to simulate
...
-   Independent external validation (gold standard) is with data that was not used in model development or calibration
...
-   Project proposal due Monday, February 22

-   Assignment 5 (last one!) to be posted by next Monday, due Wednesday March 8

-   **To discuss:** Presentation on published open science report assignment
