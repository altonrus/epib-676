...
-   Publc/non-profit-funded research is a public good
...
-  Use console when writing, but 'restart r and clear output' before final run
...
-  On new machine (with Linux), can create container based on the image
...
Put all shareable materials in one publicly-accessible, citeable spot (and cite it!)
...
-   [Global Health CEA open-source model clearinghouse](http://ghcearegistry.org/orchard/open-source-model-clearinghouse) (18 models; 4 in R)
...
## 3_intermediate and 4_output folders

-   Tables/objects generated in your scripts should go to intermediate or output folder (**not data**!)

-   Use intermediate folder for non-final objects (that are later read into another script)

    -   e.g., preprocessed data

-   For smaller projects, can get rid of intermediate and just save everything to output


## Figures

-   Save via code, not RStudio 'export'
...
-   Tweak based on saved file, not Rstudio viewer (size & filetype matter)

-   Vector format (svg, pdf) preferred over raster (usually)

    -   Vector looks sharp no matter how zoomed in
    
    -   Small filesize for line plots plots (not true for complex figures)
...
Scripts should work in new environment. Load needed packages & save-to-disk objects needed by future scripts!
...
Smaller functions calling each other easier to understand & debug vs. 1 massive function
...
Github repos private (with collaborators) while developing project, but they become public when I publish.
...
-   Should never push private data to github, even if private

::: {.callout-warning}

Objects other than data tables can include data (e.g., saving an ML model or cross-validation object often has embedded data!)
...
Can happen if two users edit same line of file, or one user edits a file  another  deletes
...
-   Reduces risk of transcription error (but may introduce new risk of coding error)
...
-   Weird hack for supplemental table/fig crossreferencing (see note in the .Qmd file)
...
::: columns
::: {.column width="50%"}
[Better BibTex for Zotero](https://retorque.re/zotero-better-bibtex/) (see [this blog](https://www.andrewheiss.com/blog/2023/01/08/bibdesk-to-zotero-pandoc/#example-workflow-with-quarto-r-markdown-pandoc)) is very helpful

-   Auto-assign citation keys and prevents duplicates withen Zenodo

-   If you export to "Better bibtex" format, with "Keep updated" checked, .bib file auto-adds anything added to the Zotero repo
:::
...
Plan ahead given close deadlines for open source model presentation, project proposal, and assignment 4.
...
-   Assignment 4 due [**Fri, Nov 2**]{.underline}
