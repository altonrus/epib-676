...
-   Vector format (svg, pdf) preferred over raster (usually)
    ...
    -   Vector always look sharp no matter how zoomed in
    
    -   For plots that are mainly lines, really small file size (but files can be too big for complex figures)
...
::: columns
::: {.column width="60%"}
-   Each script runs error-free in a new, clean environment

-   Any output that gets 'passed' to next script saved then read
...
Objects other than data tables can include data (e.g., saving an ML model 'object' often has the data embedded!)
...
-   Weird hack for supplemental table/fig crossreferencing (see note .Qmd file)
