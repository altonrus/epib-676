...
Manuscript is an advertisement, not the product.
...
-   [Global Health CEA open-source model clearinghouse](http://ghcearegistry.org/ghcearegistry/)
...
-   **pen and reproducible principles**
...
-   Put data documentation (data dictionary, meta-data) here as well

-   Can share info on data even if it's in the private folder

-   I put model parameter tables in Excel (Assign2 has good example)

    -   Read by RScript to run the model

    -   Read by manuscript Rmd/Quarto to display in maintext or supplement
...
-   Intermediate can de-clutter output, can skip for smaller projects

-   Be careful not to post 'hidden' patient level data

    -   e.g., train object from caret contains data embedded

## Figures

-   Better to save in the script vs. RStudio viewer

-   Save to file before fine-tuning

-   Vector format like svg preferred
