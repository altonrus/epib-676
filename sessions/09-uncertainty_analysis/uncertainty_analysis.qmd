...
::: {.column width="35%"}
Base case parameter value and thresholds at which optimal policy changes in OWSA shown in red.
:::
:::
...
Remember: Goal is PSA distributions of uncertainty around the mean, not variance in population!
...
The distributions used for PSA should have their [**mean value (expectation)**]{.underline} as the base case value. For asymmetric distributions, the median will not equal the mean. To simplify, I just set the base case value as the median.
...
## Preferred strategy by willingness to pay

-   Can determine preferred strategy with incremental analysis for each PSA iteration at each WTP level
...
-   In practice, we often treat PSA inputs as independent
    ...
    -   When important and feasible, better to account for correlation

## Handling corrln 1: multivariate distribution

Can fit multivariate distributions to data

-   Normal and lognormal are easiest

-   See supplement of [Fairley et. al. 2021](www.doi.org/10.1001/jamapsychiatry.2021.0247) for an example

## Handling corrln. 2: relative parameters
