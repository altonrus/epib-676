...
We'll use the relatively new [OpenTree R package from DARTH](https://github.com/DARTH-git/OpenTree/tree/main) to develop and visualize decision trees. Documentation is not fully developed, but you are encouraged to watch [this introduction video](https://vimeo.com/646960252/da5758ec62) and associated [introductory R Markdown document](https://github.com/DARTH-git/OpenTree/blob/main/Introduction%20OpenTree.Rmd).
...
# Parameters (stored in a list to keep our R environment tidy)
params <- list()
...
create_tree(file_name = "assign1_Tree", dir_name = "./assignments/assign1")


```

In the code chunk below, using the variables defined in the last code chunk (e.g., `params$p_stent_diet`), calculate the expected utility and the expected cost of the diet arm without using the `rdecision` package (summing/multiplying).
