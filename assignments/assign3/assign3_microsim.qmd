...
get_survival <- function(t_sim_cohort, #table of a (simulated) cohort of 
                         #.  individuals, which includes columns units_rbc, units_plt, 
                                       #. units_ffp and age
                         uBaseline #.  Baseline (without Zika) utility, from parameter list
                         ){
  ...
  for (row in 1:nrow(t_sim_cohort)){
    ...
    #. data.table, so I'm providing all the year 0
    #. code for you.
...
getPolOutcomes <- function(t_sim_cohort, l_params, prev=0.0001){
  ...
  #. cost_CZS and cost_zika_symptoms incorred only
