...
This code chunk contains the model. Its inputs are a vector of model parameters and a boolean (true/false) argument `project_future` with a default value of false. For calibration, we will only use past data, so you can keep the default of `project_future = false`.
...
**Model function code (No edits needed; unfold to view)**
...
## Priors & uncalibrated model

We need to create an R function that samples from the prior distribution of each parameter. For simplicity, we sample each prior parameter from an independent distribution, ignoring correlation.
...
Next, we'll generate uncalibrated results by conducting probabilistic sensitivity analysis using samples from the prior distributions. In the next two code chunks:
...
n_samples <- 500 # COMMENT OUT BEFORE SUBMITTING
# n_samples <- 1e4 #UNCOMMENT BEFORE SUBMITTING
...
Visualize your calibrated model output
...
```{r l_prior}
  ...
  # par_vector: a vector (or matrix) of model parameters (omits c)
    ...
    lprior <- dlnorm(par_vector[,1],log(0.05 )-1/2*0.5^2,0.5,log=TRUE)    # mu_e
...
We can now use these likelihoods to run SIR sampling.
...
SIR may be straightforward, but it is not very efficient: many iterations are needed to produce a fairly small number of unique parameter sets. Whereas SIR is a single-shot method, IMIS is an iterative method that builds up a better importance sampling function over different 'batches' of draws from the prior distribution for more efficient computation.
...
```{r gen_IMIS_results}
#   prior -- evaluates prior density of a parameter set or sets; can simply exponentiate the output of l_prior
prior <- function(par_vector) { 
  NA
}

#   likelihood -- evaluates likelihood of a parameter set or sets; can simply exponentiate the output of l_prior
...
#For each row of the model
for(i in 1:99) { # Change 99 to the size of imis_res$resample
  #Run the model with project_future set to T
  # extract the inc_LY and inc_cost
  NA
...
Using the `ggpairs()` function from the `GGally` package (or similar), analyze the correlation between model parameters in the m_samp (draws from the prior distribution) and then, in a separate plot, analyze the correlation between the parameter sets stored in `imis_res$resample`. Each parameter's prior distribution was sampled independently, so they should be uncorrelated with one another. Is that the case for the calibrated parameter distributions?
...
In this chunk, generate plots comparing the distribution from the model run to each #. calibration target. You can use geom_violin() to create a violin plot of the #. distribution of outcomes from the model, and use geom_pointrange to create a #. point and the 95% interval for each calibration target (based on the #. data in Table 2 above). We want to plot each calibration target point/range #. and the distribution from the model runs should be beside each other or #. on top of one another so it's clear which target goes with which outcome.

**NOTE:** For the error bar on your calibration target, you'll need to calculate confidence intervals for the prevalence data. This would be a binomial proportion confidence interval, and there are multiple ways you can calculate it in R.
...
Using the PSA parameters + outcomes data in the file \`data_for_voi.csv', compute the expected value of perfect information with a willingness-to-pay threshold of \$50,000.
